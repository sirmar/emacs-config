...
of elisp code there are a brief explaination expressing the intention of
the code.
...
#+BEGIN_SRC emacs-lisp
  (defun config-buffers ()
    ...
    (setq fourth-window (split-window first-window 270 t))
    (setq third-window (split-window first-window 180 t))
    (setq second-window (split-window first-window 90 t))
    (generate-new-buffer "*grep*")
    (set-window-buffer fourth-window "*grep*"))
...
Initialize and add the package archive for Melpa. Set proxy if needed.
...
#+BEGIN_SRC emacs-lisp
  (bind-key "C-ö" 'left-char)
  (bind-key "C-'" 'right-char)
  (bind-key "C-å" 'previous-logical-line)
  (bind-key "C-ä" 'next-logical-line)

  (bind-key "M-ö" 'left-word)
  (bind-key "M-'" 'right-word)
  (bind-key "M-å" 'backward-paragraph)
  (bind-key "M-ä" 'forward-paragraph)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (defadvice marcus-smartscan-symbol-goto (around smartscan-symbol-goto)
    (let ((case-fold-search nil))
      ad-do-it))

  (defadvice marcus-smartscan-symbol-replace (around smartscan-symbol-replace)
    (let ((case-fold-search nil))
      ad-do-it))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package smartscan
    ...
    ( ("M-n" . smartscan-symbol-go-forward)
      ...
      ("M-r" . marcus-smartscan-symbol-replace)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (setq compile-command "cd ~/code/tetris/tetris && python main.py")

  (defun marcus-run-tests ()
    (interactive)
    (when (derived-mode-p 'python-mode)
      (compile (concat "cd " (projectile-project-root) " && nosetests"))))

  (add-hook 'after-save-hook 'marcus-run-tests)
...
*** TODO Create task capture templates                            :setting:
*** TODO Consider using mail in Emacs                             :explore:
*** TODO Change theme to something nice                           :setting:
*** TODO Run tests easy.                                              :add:
*** TODO Go to test file from source file                             :add:
